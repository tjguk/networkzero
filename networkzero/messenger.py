...
def wait_for_message(address, wait_for_s=config.FOREVER):
    """Wait for a message
    
    :param address: a nw0 address (eg from `nw0.advertise`)
    :param wait_for_s: now many seconds to wait for a message before giving up
    ...
    return sockets._sockets.wait_for_message(address, wait_for_s)
